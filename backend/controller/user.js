...
const forgotPassword = wrapAsync(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "Cleaning services:- OTP For Reseting Password Recovery",
      message,
    });
  } catch (error) {
    user.otp = null;
...
const admin = wrapAsync(async (req, res) => {
  const userId = req.body.id;

  const user = await User.findById(userId);

  const { role, allServices, time, status } = req.body;
...
const changeRole = wrapAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError(`user not found with this userId :${userId}`, 401)
    );
  }
  ...
  res.status(200).json({
    success: true,
    message: "user role changed by admin Successfully",
  });
});

const getAllList = wrapAsync(async (req, res, next) => {
